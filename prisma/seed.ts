...
async function main() {
  ...
  const bauleiter = await prisma.user.upsert({
  ...
  await prisma.catalogMaterial.createMany({
    data: [
      ...
      { name: "Dachlatte 30x50mm", unit: "METER", pricePerUnit: 2.80, category: "Unterkonstruktion" },
      { name: "Mineralwolle 160mm WLG 035", unit: "QUADRATMETER", pricePerUnit: 18.50, thermalValue: 0.035, category: "Dämmung" },
      { name: "Unterspannbahn Typ A", unit: "QUADRATMETER", pricePerUnit: 3.20, category: "Abdichtung" },
      { name: "Firstziegel", unit: "STUECK", pricePerUnit: 4.50, category: "Eindeckung" },
    ],
  });

  await prisma.catalogService.createMany({
    data: [
      { name: "Dachdeckerarbeiten", unit: "STUNDE", pricePerUnit: 58.0, category: "Arbeit" },
      { name: "Gerüstbau", unit: "PAUSCHAL", pricePerUnit: 2500.0, category: "Gerüst" },
      { name: "Entsorgung Altmaterial", unit: "PAUSCHAL", pricePerUnit: 800.0, category: "Entsorgung" },
    ],
  });

  console.log("Seed-Daten erfolgreich erstellt!");
  console.log("Admin-Login: admin@handwerker.de / admin123");
  console.log("Bauleiter-Login: bauleiter@handwerker.de / user123");
}
