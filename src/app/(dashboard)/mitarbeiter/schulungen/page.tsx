...
import { Plus, AlertTriangle, CheckCircle2, Clock, GraduationCap, Trash2, Pencil, User } from "lucide-react";
...
export default function SchulungenPage() {
  ...
  async function deleteType(id: string) {
    if (!confirm("Schulungstyp wirklich löschen?")) return;
  ...
  const stats = useMemo(() => {
    ...
    for (const t of data.types) {
      for (const emp of data.employees) {
        const latest = t.records.filter((r: any) => r.userId === emp.id).sort((a: any, b: any) => new Date(b.completedAt).getTime() - new Date(a.completedAt).getTime())[0];
        if (latest?.expiresAt) {
          const exp = new Date(latest.expiresAt);
          if (exp < now) overdueCount++;
          else if (exp < soon) soonCount++;
        } else if (t.isRequired) {
          const hasAny = t.records.some((r: any) => r.userId === emp.id);
          if (!hasAny) overdueCount++;
        }
      }
    }
    return { overdueCount, soonCount, totalTypes: data.types.length, totalRecords: data.types.reduce((s: number, t: any) => s + t.records.length, 0) };
  }, [data]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Schulungen verwalten und Mitarbeiter-Nachweise überblicken</p>
        </div>
        <Button className="bg-[#9eb552] hover:bg-[#8da348] text-white" onClick={() => setCreateTypeOpen(true)}>
          <Plus className="h-4 w-4 mr-1" />Schulung anlegen
        </Button>
      </div>
                ...
                <CardContent className="p-5">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      {t.isRequired && <Badge className="bg-red-50 text-red-600 text-[10px]">Pflicht</Badge>}
                      {t.intervalMonths && <Badge variant="outline" className="text-[10px]">alle {t.intervalMonths} Monate</Badge>}
                    </div>
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="sm" onClick={() => setAddRecordOpen(t.id)} className="text-[#9eb552]"><Plus className="h-4 w-4" /></Button>
                      <Button variant="ghost" size="sm" onClick={() => setEditTypeId(t.id)}><Pencil className="h-3.5 w-3.5" /></Button>
                      <Button variant="ghost" size="sm" className="text-red-500" onClick={() => deleteType(t.id)}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </div>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b">
                          ...
                          <th className="text-left py-2 text-xs text-gray-500 font-medium">Letzte Schulung</th>
                      ...
                      <tbody>
                        {data.employees.map((emp) => {
                          const records = t.records.filter((r: any) => r.userId === emp.id).sort((a: any, b: any) => new Date(b.completedAt).getTime() - new Date(a.completedAt).getTime());
                          ...
                          const noRecord = !latest && t.isRequired;
        ...
        <Card>
          <CardContent className="p-5 overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  <th className="text-left py-2 text-xs text-gray-500 font-medium sticky left-0 bg-white">Mitarbeiter</th>
                  {data.types.map((t) => (
                    <th key={t.id} className="text-center py-2 text-xs text-gray-500 font-medium px-3 min-w-[120px]">{t.name}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {data.employees.map((emp) => (
                  <tr key={emp.id} className="border-b border-gray-50">
                    ...
                    {data.types.map((t) => {
                      const now = new Date();
                      const records = t.records.filter((r: any) => r.userId === emp.id);
                      ...
                      return (
                        <td key={t.id} className="py-2 text-center px-3">
                          {isExpired || noRecord ? (
      ...
      <Dialog open={createTypeOpen} onOpenChange={setCreateTypeOpen}>
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleCreateType} className="space-y-4">
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Beschreibung</label><Textarea name="description" rows={2} placeholder="Zusätzliche Informationen..." /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Wiederholungsintervall (Monate)</label><Input type="number" name="intervalMonths" placeholder="z. B. 24 für alle 2 Jahre" /></div>
            ...
            <Button type="submit" disabled={saving} className="w-full bg-[#9eb552] hover:bg-[#8da348] text-white">{saving ? "Anlegen..." : "Schulung anlegen"}</Button>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!editTypeId} onOpenChange={() => setEditTypeId(null)}>
        <DialogContent className="max-w-md">
          ...
          {editTypeId && (() => {
            const t = data.types.find((x) => x.id === editTypeId);
            if (!t) return null;
            return (
              <form onSubmit={handleEditType} className="space-y-4">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Bezeichnung *</label><Input name="name" required defaultValue={t.name} /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Beschreibung</label><Textarea name="description" rows={2} defaultValue={t.description || ""} /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Wiederholungsintervall (Monate)</label><Input type="number" name="intervalMonths" defaultValue={t.intervalMonths || ""} /></div>
                <label className="flex items-center gap-2"><input type="checkbox" name="isRequired" defaultChecked={t.isRequired} className="h-4 w-4 accent-[#354360] rounded" /><span className="text-sm text-gray-700">Pflichtschulung</span></label>
                <Button type="submit" disabled={saving} className="w-full bg-[#9eb552] hover:bg-[#8da348] text-white">{saving ? "Speichern..." : "Speichern"}</Button>
              </form>
            );
          })()}
        </DialogContent>
      ...
      <Dialog open={!!addRecordOpen} onOpenChange={() => setAddRecordOpen(null)}>
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleAddRecord} className="space-y-4">
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Datum der Schulung *</label><Input type="date" name="completedAt" required defaultValue={new Date().toISOString().split("T")[0]} /></div>
            ...
            <Button type="submit" disabled={saving} className="w-full bg-[#9eb552] hover:bg-[#8da348] text-white">{saving ? "Speichern..." : "Nachweis speichern"}</Button>
          </form>
        </DialogContent>
      </Dialog>
    </div>
